...
class SetupPostgresLogHintTests(unittest.TestCase):
    def completion_log_hint(self, data_dir, systemd, target_cluster=""):
        body = _function_body(
            self, SETUP_SCRIPT.read_text(encoding="utf-8"), "print_completion_message"
        )
        result = subprocess.run(
            ["bash", "-c", """
set -euo pipefail
...
has_systemd_unit_file() { [[ "${HAS_WORKING_SYSTEMD}" == "true" ]]; }
resolve_brownfield_pg_service_unit() { echo postgresql-18.service; }
...
""" + body + "\n}\nprint_completion_message\n"],
            env={
                **os.environ,
                ...
                "PG_VERSION": "18",
                "PUBLIC_ALIAS_PG_MAJOR": "18",
                "NO_ENABLE": "false",
                "USERNAME": "testadmin",
                "GATEWAY_PORT": "10260",
                "HAS_WORKING_SYSTEMD": systemd,
                "PG_STARTED_VIA": "systemd" if systemd == "true" else "pg_ctl",
                "GW_STARTED_VIA": "systemd" if systemd == "true" else "nohup",
                "TARGET_CLUSTER": target_cluster,
            },
            capture_output=True, text=True, check=False,
        )
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        match = re.search(r"^  PG logs: +(.+)$", result.stdout, re.MULTILINE)
        self.assertIsNotNone(match, result.stdout)
        return match.group(1)

    def test_collector_paths_override_startup_log_for_both_launch_modes(self):
        cases = (
            (("stderr", "log/postgresql-Fri.log"),),
            (("stderr", "/var/log/postgresql/server.log"),),
            (("csvlog", "log/server.csv"), ("jsonlog", "/var/log/postgresql/server.json")),
            (("stderr", r"log directory/server\name.log"),),
        )
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for systemd in ("false", "true"):
                for entries in cases:
                    with self.subTest(systemd=systemd, entries=entries):
                        (data_dir / "current_logfiles").write_text(
                            "".join(f"{destination} {path}\n" for destination, path in entries),
                            encoding="utf-8",
                        )
                        expected = ", ".join(
                            str(data_dir / path) for _, path in entries
                        )
                        self.assertEqual(
                            self.completion_log_hint(data_dir, systemd), expected
                        )

    def test_no_collector_and_adopted_cluster_hints_are_preserved(self):
        with tempfile.TemporaryDirectory() as tmp:
            data_dir = Path(tmp)
            for systemd in ("false", "true"):
                with self.subTest(systemd=systemd):
                    expected = (
                        "journalctl -u documentdb-postgresql@18.service"
                        if systemd == "true" else str(data_dir / "pglog.log")
                    )
                    self.assertEqual(self.completion_log_hint(data_dir, systemd), expected)
            (data_dir / "current_logfiles").write_text(
                "stderr log/postgresql-Fri.log\n", encoding="utf-8"
            )
            for systemd in ("false", "true"):
                with self.subTest(systemd=systemd, target="18/main"):
                    expected = (
                        "journalctl -u postgresql-18.service" if systemd == "true"
                        else "the adopted PostgreSQL instance's configured log"
                    )
                    self.assertEqual(
                        self.completion_log_hint(data_dir, systemd, "18/main"), expected
                    )
