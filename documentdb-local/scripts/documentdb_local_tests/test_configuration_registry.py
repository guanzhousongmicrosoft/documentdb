...
class RhelPrerequisitePinTests(unittest.TestCase):
    ...
    def _extract(self, *args):
        r = subprocess.run(["bash", str(PACKAGING / "extract-el-prereqs.sh"), *args],
                           capture_output=True, text=True, check=False)
        self.assertEqual(r.returncode, 0, r.stderr)
        return r.stdout

    def test_extractor_prints_the_readme_block(self):
        """The nightly stock-host install and the bundle summary take the block
        from extract-el-prereqs.sh, so its parse must match this one."""
        self.assertEqual(self._lines(self._extract().rstrip("\n")), self._readme_block())

    def test_extractor_rewrites_for_each_el_major_and_arch(self):
        for el, arch in (("8", "x86_64"), ("8", "aarch64"), ("9", "aarch64")):
            with self.subTest(el=el, arch=arch):
                out = self._extract("--el", el, "--arch", arch)
                for expected in (f"/EL-{el}-{arch}/", f"epel-release-latest-{el}.noarch.rpm",
                                 f"codeready-builder-for-rhel-{el}-{arch}-rpms",
                                 "--set-enabled " + ("powertools" if el == "8" else "crb")):
                    self.assertIn(expected, out)
                self.assertNotRegex(out, rf"(EL|rhel|latest)-(?!{el}\b)\d|{'x86_64' if arch == 'aarch64' else 'aarch64'}")
