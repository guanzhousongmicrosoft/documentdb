...
ADMIN_USER=""
ADMIN_PASSWORD_FILE=""
# Reviewer-flagged bug (GPT-5 iter 8): which PostgreSQL database holds
...
TARGET_DB="postgres"
# Reviewer-flagged (GPT-5.5 iter 9): see write_gateway_env_fragment.
...
# Reviewer-flagged bug (GPT-5 iter 3 + iter 4): the state file's name MATTERS
...
# Multi-reviewer flagged (should-fix H): the previous prepend/rewrite helpers
...
Options:
  ...
  --state-mode MODE     Override the auto-selected state mode. MODE must be
                        ...
                        its own lifecycle). Default: greenfield, unless
                        --target-postgres-instance is given (then brownfield).
  ...
  --admin-password-stdin  Read --admin-user password from stdin (one
                        ...
                        materializing a file:
                          printf '%s' "$PW" | sudo documentdb-register-gateway \
                            ... --admin-user admin --admin-password-stdin
...
strip_managed_block() {
    local f="$1" s="$2" e="$3"
    [[ -f "$f" ]] || return 0
    if grep -Fqx "$s" "$f" 2>/dev/null; then
        if ! grep -Fqx "$e" "$f" 2>/dev/null; then
            echo "${PROG}: refusing to strip managed block in $f: end marker missing." >&2
            cat "$f"
            return 0
        fi
    fi
    awk -v s="$s" -v e="$e" '$0==s{k=1;next} $0==e{k=0;next} !k{print}' "$f"
}

extract_managed_block_content() {
    local f="$1" s="$2" e="$3"
    [[ -f "$f" ]] || return 0
    awk -v s="$s" -v e="$e" '$0==s{b=1;next} $0==e{b=0;next} b{print}' "$f"
}

# Prepend managed block before the first non-comment, non-blank line.
# Important for pg_hba.conf where rule order matters (first match wins).
prepend_with_managed_block() {
    local target_file="$1" start="$2" end="$3" content="$4"
    local stripped tmp block_file
    local target_dir
    target_dir="$(dirname "${target_file}")"

    create_temp_in_dir stripped "${target_dir}"
    create_temp_in_dir tmp "${target_dir}"
    create_temp_in_dir block_file "${target_dir}"

    strip_managed_block "${target_file}" "${start}" "${end}" > "${stripped}"
    ...
    if [[ -z "${content}" ]]; then
        ...
        awk -v bf="${block_file}" '
            ...
            !ins && /^[^#]/ && !/^[[:space:]]*$/ {
                ...
                close(bf); print ""; ins=1
            }
            { print }
            END { if (!ins) { while ((getline l < bf) > 0) print l; close(bf) } }
        ' "${stripped}" > "${tmp}"
    fi

    if [[ -e "${target_file}" ]]; then
        chown --reference="${target_file}" "${tmp}" 2>/dev/null || true
...
rewrite_with_managed_block() {
    local target_file="$1" start="$2" end="$3" content="$4"
    local _stripped _tmp
    local target_dir
    target_dir="$(dirname "${target_file}")"

    create_temp_in_dir _stripped "${target_dir}"
    create_temp_in_dir _tmp "${target_dir}"

    strip_managed_block "${target_file}" "${start}" "${end}" > "${_stripped}"
    ...
    if [[ -e "${target_file}" ]]; then
        chown --reference="${target_file}" "${_tmp}" 2>/dev/null || true
        chmod --reference="${target_file}" "${_tmp}" 2>/dev/null || true
    fi
    mv "${_tmp}" "${target_file}"
    rm -f "${_stripped}"
}
...
resolve_cluster_paths() {
    if [[ -n "${PGDATA}" ]]; then
        ...
        # Reviewer-flagged (external review iter 18): without this
        # SOCKET_DIR default, the early-return left it empty when
        # --pgdata was the only path hint. Later role-creation and
        # extension-presence checks at lines ~522-536 require
        # SOCKET_DIR + PG_PORT and silently skip without it. The
        # written pg-url then has `host=&port=...` (empty host),
        # which the gateway treats as an invalid connection string.
        # /var/run/postgresql is the convention on both Debian and
        # RHEL packaged PostgreSQL — it matches the fallbacks below
        # for named-cluster discovery. Operators using a non-default
        # socket dir must still pass --socket-dir explicitly.
...
resolve_target_cluster_port() {
    if [[ "${IS_DEBIAN}" == "true" ]]; then
        ...
        if [[ -r "${rhel_conf}" ]]; then
            local discovered_port
            discovered_port="$(awk -F= '/^[[:space:]]*port[[:space:]]*=/{gsub(/^[[:space:]]+/, "", $2); gsub(/[[:space:]#].*/, "", $2); print $2; exit}' "${rhel_conf}")"
            if [[ -n "${discovered_port}" ]]; then
                PG_PORT="${discovered_port}"
...
# Back up a file with a timestamped copy before modifying it.
backup_file() {
    local file="$1"
    local backup_dir="${2:-}"
    [[ -f "${file}" ]] || return 0
    ...
    local backup_file
    backup_file="$(mktemp "${backup_dir}/$(basename "${file}").documentdb-backup.${timestamp}.XXXXXX")" \
        || die "Cannot create backup file for ${file} in ${backup_dir}."
    cp -a "${file}" "${backup_file}"
    log "Backed up ${file} -> ${backup_file}"
}

# Refuse to modify a config file containing managed-block markers from
# another tool (not ours). Prevents silent overwrite of foreign state.
check_foreign_markers() {
    local file="$1" our_start="$2"
    [[ -f "${file}" ]] || return 0

    local foreign_count
    foreign_count="$(grep -cE '^\s*#\s*>>>\s+\S+\s+managed\s+' "${file}" 2>/dev/null || true)"
    local our_count
    our_count="$(grep -cFx "${our_start}" "${file}" 2>/dev/null || true)"

    if (( foreign_count > our_count )); then
        die "${file} contains managed-block markers from another tool. Refusing to modify. Inspect the file manually."
    fi
}
...
do_setup() {
    ...
    # in main() before any I/O. The duplicate check here is intentional
    # belt-and-suspenders: any future direct caller of do_setup() would
    # otherwise be able to bypass the prereq.
    ...
    # Reviewer-flagged bug (GPT-5 iter 2): persist a minimal recovery record
    # BEFORE the first config mutation, not after the whole flow succeeds.
    # Reviewer-flagged bug (GPT-5 iter 3): the port resolver narrows the
    ...
    #
    # Reviewer-flagged (GPT-5.4 iter 10): MUST run BEFORE write_recovery_marker
    ...
    # Reviewer-flagged (GPT-5 iter 2): persist a minimal recovery record
    # BEFORE the first config mutation, not after the whole flow succeeds.
    ...
    check_foreign_markers "${HBA_FILE}" "${PG_HBA_BLOCK_START}"
    check_foreign_markers "${IDENT_FILE}" "${PG_IDENT_BLOCK_START}"
    ...
    # WARNING is noise that looks like an error (Gap #9). Suppress under
    ...
    # Reviewer-flagged (multi-model should-fix M): track success so the
    ...
    # Real-user E2E flagged (Gap #4): when this tool is run as a delegated
    ...
    # Reviewer-flagged (external review iter 17 + multi-model should-fix M):
    ...
    if [[ "${admin_bootstrap_succeeded}" == "true" ]]; then
        local connect_host="127.0.0.1:10260"
        ...
        connect_uri+="${ADMIN_USER}@${connect_host}/?${connect_opts}"
        ...
        echo "  4. Then connect with that user via mongosh on localhost:10260 (tls=true)."
    else
...
create_gateway_role() {
    ...
    role_exists="$(run_as_user "${PG_OWNER}" "${PSQL}" -h "${SOCKET_DIR}" -p "${PG_PORT}" \
        ...
        -c "SELECT 1 FROM pg_roles WHERE rolname = '${GW_PG_ROLE}';" 2>&1)" || \
...
write_connection_secret() {
    ...
    # Reviewer-flagged (GPT-5 iter 8): use ${TARGET_DB} (set by --target-db,
...
record_state() {
    if [[ -n "${STATE_FILE}" ]]; then
        ...
        # Reviewer-flagged (GPT-5.5 iter 14): write atomically via
        ...
        # Reviewer-flagged (GPT-5.5 iter 15): preserve any KEY=VALUE
        ...
        # Reviewer-flagged (external review iter 18): also include
        # TARGET_DB in the managed set so we own that field.
        local managed_keys_re='^(HBA_FILE|IDENT_FILE|SECRET_FILE|PG_VERSION|CLUSTER_NAME|PG_PORT|PG_OWNER|DOCUMENTDB_MODE|GATEWAY_ENV_FILE|TARGET_DB)='
        local preserved=""
...

        {
            ...
            # commands at the right cluster. Before iter 4 these were
            ...
            # Reviewer-flagged (Sonnet iter 13): GATEWAY_ENV_FILE is chosen
            ...
            # Reviewer-flagged (external review iter 18): TARGET_DB
...
# Reviewer-flagged bug (GPT-5 iter 2): persist a minimal recovery marker
...
write_recovery_marker() {
    ...
    install -d -m 0755 "$(dirname "${STATE_FILE}")"
    local tmp
    # Reviewer-flagged (GPT-5 iter 3): treat mktemp failure as fatal. The
    ...
    # Reviewer-flagged (GPT-5.5 iter 15, applied to recovery marker too):
...

    {
        ...
        printf 'PG_VERSION=%s\n' "${PG_VERSION}"
        printf 'CLUSTER_NAME=%s\n' "${CLUSTER_NAME}"
        # Reviewer-flagged (Sonnet iter 14 S-1): also persist
...
# Reviewer-flagged (GPT-5 iter 3): after resolve_target_cluster_port set
...
verify_psql_connects_to_named_cluster() {
    # Only meaningful when we know which cluster was named and we have psql.
    [[ -n "${PG_VERSION}" && -n "${CLUSTER_NAME}" ]] || return 0
    [[ -n "${PSQL}" && -n "${SOCKET_DIR}" && -n "${PG_PORT}" ]] || return 0

    local live_config_file live_hba_file live_ident_file
    ...
    # Reviewer-flagged (GPT-5.4 iter 9): resolve the live hba_file and
    ...
    log_verbose "Cluster identity verified: live config_file=${live_config_file} is under the named cluster's config tree (hba=${HBA_FILE}, ident=${IDENT_FILE})."
}
...
do_restore() {
    ...
    # Reviewer-flagged (multi-model blocker I + Sonnet iter 13, GPT-5.5 iter
    # 14): load EVERY persisted path from the state file before doing any
    ...
    # Reviewer-flagged (GPT-5.5 iter 11): clean the legacy /run/ path
    # too in case --restore is run on a host with a pre-iter11 install.
    if [[ -n "${PG_VERSION}" ]]; then
        local legacy_secret="/run/documentdb-local/${PG_VERSION}/gateway/pg-url"
    ...
    # Remove the managed block from the gateway env file (preserving any
    # administrator-authored lines outside the block).
    # Reviewer-flagged (GPT-5.5 iter 14): when the state file did not
    # carry GATEWAY_ENV_FILE (pre-iter13 install), strip from BOTH known
...
parse_arguments() {
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --pgdata)
                ...
                PGDATA="$2"; shift 2 ;;
            --pg-version)
                # Reviewer-flagged (GPT-5.4 iter 11): when called via
            ...
            --target-db)
                ...
                # delegation to documentdb-gateway-admin. Reviewer-flagged
                # (GPT-5 iter 8): without this flag the gateway was
                # silently locked to /postgres regardless of where
                # CREATE EXTENSION actually ran.
    ...
    if [[ -n "${TARGET_CLUSTER}" ]]; then
        ...
        [[ "${CLUSTER_NAME}" =~ ^[a-zA-Z0-9_-]+$ ]] || die "Invalid cluster name '${CLUSTER_NAME}'. Only alphanumerics, hyphens, and underscores are allowed."
    ...
    # Reviewer-flagged (GPT-5.4 iter 11): when called with --pgdata only
    ...
    # Reviewer-flagged (GPT-5 iter 4): auto-default --state-mode so an
    ...
    #   - --target-postgres-instance set → brownfield (adopting a named distro cluster)
    #   - otherwise                      → greenfield (private PG via --pgdata
    #                                      or single-instance auto-detect for Workflow B)
    if [[ "${STATE_MODE_EXPLICIT}" != "true" ]]; then
        if [[ -n "${TARGET_CLUSTER}" ]]; then
            STATE_MODE="brownfield"
            log_verbose "Auto-selected --state-mode brownfield (--target-postgres-instance is set)."
        else
            STATE_MODE="greenfield"
            log_verbose "Auto-selected --state-mode greenfield (no --target-postgres-instance)."
        fi
    fi
}
...
main() {
    ...
    # Reviewer-flagged bug (GPT-5 iter 2): when --target-postgres-instance
    ...
    #
    # Reviewer-flagged (GPT-5.5 iter 11): the original design path of
