...
pub async fn run_server(
    ...
    let listener = TcpListener::bind(format!(
        ...
        sc.setup_configuration().mongo_listen_port.unwrap_or(27017)
    ))
    .await?;
...
pub async fn get_service_context(
    ...
    mut dynamic: Option<Box<dyn DynamicConfiguration>>,
    ...
    loop {
        interval.tick().await;
        match ServiceContext::new(&setup_configuration, &mut dynamic, &query_catalog).await {
            Ok(sc) => return Ok(sc),
...
async fn get_response(
    ...
    if request.request_type() != &RequestType::IsMaster {
        ...
        if log::log_enabled!(log::Level::Trace) {
            log::trace!("Request: {:?} - {}", message.op_code, request.to_json()?);
        }
    }
    ...
    R: AsyncRead + AsyncWrite + Unpin + Send,
{
    ...
    log::trace!("[{}] Request parsed.", header.request_id);
    ...
    if log_enabled!(log::Level::Trace) {
        log::trace!("Request: {}", request.to_json()?)
    }
    ...
    R: AsyncRead + AsyncWrite + Unpin + Send,
{
    ...
    *collection = request_info.collection().unwrap_or("").to_string();
    ...
    let start = compute_request_tracker.start_timer();
    let response = get_response(ctx, message, request, &request_info).await?;
