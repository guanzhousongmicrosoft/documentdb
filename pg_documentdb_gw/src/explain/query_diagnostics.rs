...
pub fn is_output_count(output: &str, query_catalog: &QueryCatalog) -> bool {
    let output = output.replace(query_catalog.is_output_count_regex_1(), "");
    ...
    if output.contains(query_catalog.is_output_count_regex_2()) {
        return true;
    }
...
fn get_expressions(
    ...
    for captures in captures {
        if let Some(capture) = captures.name("expr") {
            ...
            {
                let operator =
                    mongo_operator(index_condition.name("operator").map_or("", |m| m.as_str()));
...
pub fn get_mongo_runtime_conditions(
    input: &str,
    query_catalog: &QueryCatalog,
) -> Vec<(&'static str, RawDocumentBuf)> {
    get_expressions(
...
pub fn get_mongo_index_conditions(
    input: &str,
    query_catalog: &QueryCatalog,
) -> Vec<(&'static str, RawDocumentBuf)> {
    get_expressions(
...
fn mongo_operator(input: &str) -> &'static str {
    match input {
        "@=" => "$eq",
        "@>" => "$gt",
        "@>=" => "$gte",
