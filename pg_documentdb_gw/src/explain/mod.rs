...
async fn run_explain(
    ...
    let dynamic_config = connection_context.dynamic_configuration().await;
    let results = if matches!(
        ...
        t.batch_execute("SET LOCAL citus.explain_all_tasks to true;")
            .await?;
        let stmt = t
    ...
    match plan.node_type.as_str() {
        ...
        "Bitmap Heap Scan" | "Parallel Bitmap Heap Scan" => {
            if plan
                .filter
                .as_ref()
                .is_some_and(|f| f.contains(query_catalog.find_bson_test_meta_qual()))
            {
        ...
        "Custom Scan" => {
            if let Some(cpp) = plan.custom_plan_provider.as_ref() {
                match cpp.as_str() {
                    ...
                    | "HelioApiQueryScan"
                    | "DocumentDBApiScan"
                    | "DocumentDBApiQueryScan" => ("FETCH".to_owned(), None),
                    _ => {
    ...
    let result = walk_plan_stage(
        ...
        |plan, stage_name, query_catalog| {
            ...
            if let Some(filter) = plan.filter.as_deref() {
                let values = query_diagnostics::get_mongo_runtime_conditions(filter, query_catalog);
                if !values.is_empty() {
            ...
            if plan.index_condition.is_some() && stage_name != "FETCH" {
                let conditions = query_diagnostics::get_mongo_index_conditions(
