...
pub fn pg_configuration(sc: &SetupConfiguration) -> tokio_postgres::Config {
    let mut config = tokio_postgres::Config::new();
    config
        .host(sc.postgres_host_name.as_deref().unwrap_or("localhost"))
        .port(sc.postgres_port.unwrap_or(9712))
        .dbname(sc.postgres_database.as_deref().unwrap_or("postgres"));
    config
}
