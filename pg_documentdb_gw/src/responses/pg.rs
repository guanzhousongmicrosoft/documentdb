...
impl PgResponse {
    ...
    /// In certain cases, write errors need to be walked through to convert PG error codes to mongo ones.
                ...
                .await
        {
                ...
                || known == ErrorCode::Unauthorized as i32
            {
                return Err(DocumentDBError::UntypedDocumentDBError(
                    ...
                    error_code.unwrap_or("Mongo Error").to_string(),
    ...
    pub async fn known_pg_error(
        ...
        match *state {
            ...
            SqlState::READ_ONLY_SQL_TRANSACTION if connection_context.dynamic_configuration().await.is_replica_cluster() => {
    ...
    /// Smallest value of the pgmongo backend error (DocumentDBError.ok). All error codes greater than this will be Mongo native errors.
    pub const API_ERROR_CODE_MIN: i32 = 687865856;

    /// Corresponds to the PG ErrorCode 000PY.
    /// Largest value of the pgmongo backend error.
