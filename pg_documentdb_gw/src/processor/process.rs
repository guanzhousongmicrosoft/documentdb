    ...
    let result = loop {
        let response = match request.request_type() {
            ...
            RequestType::IsMaster => {
                ismaster::process("ismaster", request, connection_context, &dynamic_config)
            }
        ...
        let retry = match &response {
            ...
            Err(DocumentDBError::PostgresError(error, _)) => {
                retry_policy(&dynamic_config, error, request.request_type())
            }
...
async fn process_coll_stats(
    ...
    // Mongo allows floats and ints, the backend will truncate
