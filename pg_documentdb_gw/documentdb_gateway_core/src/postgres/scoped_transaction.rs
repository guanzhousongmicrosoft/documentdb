...
use crate::postgres::conn_mgmt::{Connection, StatementError};
...
impl ScopedTransaction {
    ...
    ///
    /// # Errors
    /// Returns a [`StatementError`] if issuing `START TRANSACTION` fails.
    pub async fn start_if_necessary(
        connection: &Arc<Connection>,
    ) -> std::result::Result<Self, StatementError> {
    ...
    ///
    /// # Errors
    /// Returns a [`StatementError`] if the `COMMIT` statement fails.
    pub async fn commit(&mut self) -> std::result::Result<(), StatementError> {
...
impl Drop for ScopedTransaction {
    fn drop(&mut self) {
        if let Some(conn) = self.connection.take() {
            // Best-effort async rollback, since `Drop` cannot await. The
            // transaction flag stays set until the `ROLLBACK` is acknowledged,
            // so the `Connection::drop` backstop still fires if this task never
            // runs.
