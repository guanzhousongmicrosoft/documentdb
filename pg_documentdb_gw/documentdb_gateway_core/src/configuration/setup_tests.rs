...
#[test]
fn timeouts_deserialize_from_setup_configuration() {
    let configuration: DocumentDBSetupConfiguration = serde_json::from_value(serde_json::json!({
        "NodeHostName": "localhost",
        "BlockedRolePrefixes": [],
        "CertificateOptions": { "CertType": "PemAutoGenerated" },
        "TransactionTimeoutSecs": 5,
        "PostgresCommandTimeoutSecs": 45,
    }))
    .expect("timeout setup configuration should deserialize");

    assert_eq!(
        SetupConfiguration::transaction_timeout_secs(&configuration),
        Some(5)
    );
    assert_eq!(
        SetupConfiguration::postgres_command_timeout_secs(&configuration),
        Some(45)
    );

    let defaults = DocumentDBSetupConfiguration::default();
    assert_eq!(
        SetupConfiguration::transaction_timeout_secs(&defaults),
        None
    );
    assert_eq!(
        SetupConfiguration::postgres_command_timeout_secs(&defaults),
        None
    );
}
