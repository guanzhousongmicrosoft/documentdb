...
(
    ...
    PARALLEL = RESTRICTED
    -- Parallel-restricted: the bsonaggvalue transition state embeds raw pointers and
    -- cannot survive partial aggregation across a worker boundary; RESTRICTED keeps
    -- the aggregate on the leader (no partial aggregation) while still allowing a
    -- parallel scan beneath the Gather. Use BSONMAXWITHEXPRINTERNAL
    -- for a parallel-safe variant.
...
(
    ...
    -- cannot survive partial aggregation across a worker boundary; RESTRICTED keeps
    -- the aggregate on the leader (no partial aggregation) while still allowing a
    -- parallel scan beneath the Gather. Use BSONMINWITHEXPRINTERNAL
    -- for a parallel-safe variant.
    -- No MSSFUNC/MFINALFUNC/MINVFUNC as this aggregate is not invertable
);
...
CREATE OR REPLACE AGGREGATE __API_SCHEMA_INTERNAL_V2__.BSONFIRSTWITHEXPR(document __CORE_SCHEMA__.bson, expressionSpec __CORE_SCHEMA__.bson, variableSpec __CORE_SCHEMA__.bson, collationString text)
(
    SFUNC = __API_SCHEMA_INTERNAL_V2__.bson_first_with_expr_transition,
    FINALFUNC = __API_SCHEMA_INTERNAL_V2__.bson_first_with_expr_final,
    stype = __API_SCHEMA_INTERNAL_V2__.bsonaggvalue,
    COMBINEFUNC = __API_SCHEMA_INTERNAL_V2__.bson_first_with_expr_combine,
    PARALLEL = RESTRICTED
    -- Parallel-restricted: the bsonaggvalue transition state embeds raw pointers and
    -- cannot survive partial aggregation across a worker boundary; RESTRICTED keeps
    -- the aggregate on the leader (no partial aggregation) while still allowing a
    -- parallel scan beneath the Gather. Use BSONFIRSTWITHEXPRINTERNAL
    -- for a parallel-safe variant.
    -- No MSSFUNC/MFINALFUNC/MINVFUNC as this aggregate is not invertible
);

CREATE OR REPLACE AGGREGATE __API_SCHEMA_INTERNAL_V2__.BSONLASTWITHEXPR(document __CORE_SCHEMA__.bson, expressionSpec __CORE_SCHEMA__.bson, variableSpec __CORE_SCHEMA__.bson, collationString text)
(
    SFUNC = __API_SCHEMA_INTERNAL_V2__.bson_last_with_expr_transition,
    FINALFUNC = __API_SCHEMA_INTERNAL_V2__.bson_last_with_expr_final,
    stype = __API_SCHEMA_INTERNAL_V2__.bsonaggvalue,
    COMBINEFUNC = __API_SCHEMA_INTERNAL_V2__.bson_last_with_expr_combine,
    PARALLEL = RESTRICTED
    -- Parallel-restricted: the bsonaggvalue transition state embeds raw pointers and
    -- cannot survive partial aggregation across a worker boundary; RESTRICTED keeps
    -- the aggregate on the leader (no partial aggregation) while still allowing a
    -- parallel scan beneath the Gather. Use BSONLASTWITHEXPRINTERNAL
    -- for a parallel-safe variant.
    -- No MSSFUNC/MFINALFUNC/MINVFUNC as this aggregate is not invertible
);
...
-- Parallel-safe internal-state variants of the $max/$min with-expr aggregates.
-- The transition state is kept as an "internal" pointer and is flattened to a
-- self-contained byte buffer by the serialize function so it can be moved across
-- a parallel worker boundary, then rebuilt by the deserialize function.
CREATE OR REPLACE AGGREGATE __API_SCHEMA_INTERNAL_V2__.BSONMAXWITHEXPRINTERNAL(document __CORE_SCHEMA__.bson, expressionSpec __CORE_SCHEMA__.bson, variableSpec __CORE_SCHEMA__.bson, collationString text)
(
    SFUNC = __API_SCHEMA_INTERNAL_V2__.bson_max_with_expr_transition_internal,
    FINALFUNC = __API_SCHEMA_INTERNAL_V2__.bson_min_max_with_expr_final_internal,
    stype = internal,
    COMBINEFUNC = __API_SCHEMA_INTERNAL_V2__.bson_max_with_expr_combine_internal,
    SERIALFUNC = __API_SCHEMA_INTERNAL_V2__.bson_agg_value_serialize,
    DESERIALFUNC = __API_SCHEMA_INTERNAL_V2__.bson_agg_value_deserialize,
    PARALLEL = SAFE
    -- No MSSFUNC/MFINALFUNC/MINVFUNC as this aggregate is not invertable
);

CREATE OR REPLACE AGGREGATE __API_SCHEMA_INTERNAL_V2__.BSONMINWITHEXPRINTERNAL(document __CORE_SCHEMA__.bson, expressionSpec __CORE_SCHEMA__.bson, variableSpec __CORE_SCHEMA__.bson, collationString text)
(
    SFUNC = __API_SCHEMA_INTERNAL_V2__.bson_min_with_expr_transition_internal,
    FINALFUNC = __API_SCHEMA_INTERNAL_V2__.bson_min_max_with_expr_final_internal,
    stype = internal,
    COMBINEFUNC = __API_SCHEMA_INTERNAL_V2__.bson_min_with_expr_combine_internal,
    SERIALFUNC = __API_SCHEMA_INTERNAL_V2__.bson_agg_value_serialize,
    DESERIALFUNC = __API_SCHEMA_INTERNAL_V2__.bson_agg_value_deserialize,
    PARALLEL = SAFE
    -- No MSSFUNC/MFINALFUNC/MINVFUNC as this aggregate is not invertable
...
-- Parallel-safe internal-state variants of the $first/$last with-expr aggregates.
-- The transition state is kept as an "internal" pointer and flattened to a
-- self-contained byte buffer by the shared bson_agg_value_serialize function so
-- it can be moved across a parallel worker boundary, then rebuilt by
-- bson_agg_value_deserialize.
CREATE OR REPLACE AGGREGATE __API_SCHEMA_INTERNAL_V2__.BSONFIRSTWITHEXPRINTERNAL(document __CORE_SCHEMA__.bson, expressionSpec __CORE_SCHEMA__.bson, variableSpec __CORE_SCHEMA__.bson, collationString text)
(
    SFUNC = __API_SCHEMA_INTERNAL_V2__.bson_first_with_expr_transition_internal,
    FINALFUNC = __API_SCHEMA_INTERNAL_V2__.bson_first_with_expr_final_internal,
    stype = internal,
    COMBINEFUNC = __API_SCHEMA_INTERNAL_V2__.bson_first_with_expr_combine_internal,
    SERIALFUNC = __API_SCHEMA_INTERNAL_V2__.bson_agg_value_serialize,
    DESERIALFUNC = __API_SCHEMA_INTERNAL_V2__.bson_agg_value_deserialize,
    PARALLEL = SAFE
    -- No MSSFUNC/MFINALFUNC/MINVFUNC as this aggregate is not invertible
...
CREATE OR REPLACE AGGREGATE __API_SCHEMA_INTERNAL_V2__.BSONLASTWITHEXPRINTERNAL(document __CORE_SCHEMA__.bson, expressionSpec __CORE_SCHEMA__.bson, variableSpec __CORE_SCHEMA__.bson, collationString text)
(
    SFUNC = __API_SCHEMA_INTERNAL_V2__.bson_last_with_expr_transition_internal,
    FINALFUNC = __API_SCHEMA_INTERNAL_V2__.bson_last_with_expr_final_internal,
    stype = internal,
    COMBINEFUNC = __API_SCHEMA_INTERNAL_V2__.bson_last_with_expr_combine_internal,
    SERIALFUNC = __API_SCHEMA_INTERNAL_V2__.bson_agg_value_serialize,
    DESERIALFUNC = __API_SCHEMA_INTERNAL_V2__.bson_agg_value_deserialize,
    PARALLEL = SAFE
    -- No MSSFUNC/MFINALFUNC/MINVFUNC as this aggregate is not invertible
