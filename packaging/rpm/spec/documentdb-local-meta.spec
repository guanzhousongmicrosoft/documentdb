...
To fully uninstall, run:
  sudo dnf remove documentdb documentdb-%{default_pg_major} \
...
%post
# Reviewer-flagged (GPT-5 iter 5): the DEB meta package's postinst
...
#
# Reviewer-flagged (GPT-5 iter 7): a plain `Requires=` wrapper unit
...
# package's %post. Removed on full uninstall.
...
%postun
# On full removal ($1 == 0 in RPM), remove the alias we installed in %post.
