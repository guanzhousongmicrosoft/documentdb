...
if [[ "$PACKAGE_TYPE" == "deb" ]]; then
    # noddebs stops dh_strip from emitting the automatic -dbgsym package.
    # debuild preserves DEB_* variables through its environment sanitizing, so
    # the value set here reaches dh inside the container; an empty value is a
    # no-op.
    DEB_BUILD_OPTIONS_VALUE=""
    if [[ $NO_DBGSYM == true ]]; then
        DEB_BUILD_OPTIONS_VALUE="noddebs"
    fi
