...
/scripts/documentdb-tune.sh --pgdata "$tmpdir" --yes 2>&1 | grep -q "already up to date"
if [ $? -eq 0 ]; then
...
# ── Test 8: documentdb-gateway wrapper ──────────────────────────────
echo "--- Test 8: documentdb-gateway --help ---"
if /scripts/documentdb-gateway-wrapper.sh --help 2>&1 | grep -q "documentdb-gateway"; then
    pass "documentdb-gateway --help works"
else
    fail "documentdb-gateway --help failed"
fi

# ── Test 9: documentdb-register-gateway --help ──────────────────────
echo "--- Test 9: documentdb-register-gateway --help ---"
if /scripts/documentdb-gateway-wrapper.sh setup --help 2>&1 | grep -q "target-postgres-instance"; then
    pass "documentdb-register-gateway --help works"
else
    fail "documentdb-register-gateway --help failed"
fi

# ── Test 10: documentdb-gateway-admin --help ────────────────────────
echo "--- Test 10: documentdb-gateway-admin --help ---"
if /scripts/documentdb-gateway-wrapper.sh admin --help 2>&1 | grep -q "create-user"; then
...
# ── Test 14: removed — documentdb-local.sh was deleted (multi-reviewer flagged
#    should-fix A: a CLI whose only subcommand always errors is bad UX). Public
